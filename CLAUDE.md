...
tailored-resumes-feature/       Self-contained resume tailoring tool (see its README.md)
  ...
  scripts/
    ...
    ats.py                      ATS keyword extraction and coverage scoring; also an audit CLI
...
# 4. Audit any resume against a job description's keywords (no API call)
python $T/ats.py salesforce                      # tailored resume, or the base PDF
python $T/ats.py salesforce --resume Du_Tyler_Resume_Base.pdf
python $T/ats.py salesforce --all                # every extracted keyword, matched and missed
```
...
**ATS keyword targeting** — `tailored-resumes-feature/scripts/ats.py` models the dumb string matcher that filters a resume before a person sees it. No model involved: it extracts terms from a job description (a curated skills dictionary in `SKILLS`, plus a CamelCase/acronym scan that catches tools the dictionary has never heard of), tiers each one by how hard the description leans on it (`hits >= 3`, or `>= 2` inside a requirements block, is `critical`), then string-matches them against a resume. Matching is deliberately tolerant of spelling (`on-call` finds `on call` and `oncall`, `troubleshoot` finds `troubleshooters`) and deliberately strict on boundaries (`Java` never fires on `JavaScript`); overlapping aliases are counted once. `tailor_resume.py` injects the ranked list into the prompt (`ats.prompt_block`) and, in the same retry loop that fixes page overflow, re-asks when a draft misses a critical term or three important ones. Page fit stays the hard constraint: an overflowing draft never wins on keywords. The `domain`, `traits`, and `logistics` categories are scored but never sent back to the model, since no rewording earns an industry Tyler has not worked in. Coverage lands in the tailored Markdown as an **ATS Keyword Coverage** section, and `render_resume.py` re-scores hand-edited files so an edit that drops a keyword is visible. `ats.py` also runs standalone to audit any resume, including the untailored base PDF. The knockout checks (`ats.knockouts`) cover the rules that reject a resume before scoring: no phone number, no work authorization line when the description raises it, and a location outside an onsite role's radius.
